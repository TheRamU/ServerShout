...
/**
 * @author TheRamU
 * @since 2024/8/28 16:51
 */
class BalanceTakeCommandHandler : CommandHandler {
    ...
    override val arguments = arrayOf("balance", "take", "<NAME>", "<TOKEN>", "<AMOUNT>")
    override val permission = "servershout.balance.take"
    override val playerOnly = false
    override val usage get() = "message.commands.balance-take.usage"
    ...
    override fun handle(sender: PlatformCommandSender, alias: String, args: Array<String>) {
        ...
        balanceService.takeBalance(playerName, token, amount)
        val balanceAmount = balanceService.getBalanceAmount(playerName, token)
        sender.sendLanguageMessage("message.commands.balance-take.success", playerName, tokenName, amount, balanceAmount)
